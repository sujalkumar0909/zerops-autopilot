...
# Matches "Dockerfile", "dockerfile", "backend.Dockerfile", "Dockerfile.prod",
# "api.containerfile", etc. - not just the exact literal filename "Dockerfile",
# since real projects commonly prefix/suffix it per-service.
...
def detect_manifest_type(directory_files):
    ...
    if "package.json" in names:
        return "Node.js"
    if names.intersection({"requirements.txt", "pyproject.toml", "pipfile"}):
        return "Python"
    if names.intersection({"pom.xml", "build.gradle", "build.gradle.kts"}):
        return "Java"
    if "go.mod" in names:
        return "Go"
    ...
    if any(name.endswith(".csproj") for name in names):
        return "C#"
    # Note: a Dockerfile's presence alone doesn't make a directory an
    # application service - a shared "docker/" folder holding per-service
    # Dockerfiles is common, and isn't itself a service. Only a real
    # language manifest (handled above) establishes a service directory.
    return "Unknown"
...
def detect_service_technology(directory_files, archive):
    ...
    if manifest == "Node.js":
        ...
        if frameworks:
            return "Node.js / " + " + ".join(frameworks)
        return "Node.js"
    if manifest == "Python":
        frameworks = detect_frameworks(directory_files, archive)
        if frameworks:
            return "Python / " + " + ".join(frameworks)
        return "Python"
    return manifest
...
def detect_technologies(files, archive):
    ...
    if any(is_dockerfile_name(basename(file)) for file in files):
    ...
    if any(
        ...
        and re.search(
            ...
            read_text_from_zip(archive, file, 700_000)
        )
        for file in files
    ):
...
def parse_compose_services(archive, compose_files):
    """Lightweight structural parser for docker-compose 'services:' blocks.

    Captures image/build (including the nested `build: {context, dockerfile}`
    mapping form, which is the common real-world style), ports, depends_on
    and replicas, in a single pass over the indentation structure.
    """
    ...
    for file in compose_files:
        ...
        in_services = False
        current = None
        block = None  # which nested list/mapping we're currently inside
        ...
        for raw in text.splitlines():
            ...
            if indent == 2 and stripped.endswith(":") and not stripped.startswith("-"):
                ...
                if name:
                    ...
                    services.setdefault(name, {
                        ...
                        "build": None, "build_context": None, "dockerfile": None,
                        "ports": [], "depends_on": [], "replicas": 1
                    })
                block = None
                continue
            ...
            if indent == 4:
                if stripped.startswith("image:"):
                    ...
                    if value:
                        # Inline form: build: ./service-dir
                        entry["build"] = value
                        entry["build_context"] = value
                        block = None
                    else:
                        # Nested mapping form: build:\n  context: ...\n  dockerfile: ...
                        block = "build"
                    ...
                    if replicas:
                        entry["replicas"] = int(replicas.group(1))
                    block = None
            ...
            if indent >= 6 and block == "depends_on":
                ...
                if re.match(r"^[A-Za-z0-9_.-]+$", dependency):
                    entry["depends_on"].append(dependency)
                continue
...
def classify_directory(name, technology, directory_files, archive):
    ...
    if any(term in text for term in (
        "bullmq", "celery", "rq.worker", "consumer(", "kafkaconsumer",
        "rabbitmq", "background_tasks", "dramatiq"
    )):
        return "worker"
...
def compose_service_target_hints(item):
    """Directory-name-like hints for what a compose service actually builds,
    derived from its own config rather than any fixed name list."""

    hints = []

    context = clean_path(item.get("build_context") or "").strip("/")
    if context and context not in ("", "."):
        # e.g. build.context: "./backend" -> "backend"
        hints.append(directory_name(context))

    dockerfile = clean_path(item.get("dockerfile") or "")
    if dockerfile:
        # e.g. "docker/backend.Dockerfile" -> "backend"
        stem = os.path.splitext(os.path.basename(dockerfile))[0]
        if stem:
            hints.append(stem.lower())
        # e.g. "backend/Dockerfile" -> "backend" (the containing directory)
        parent = os.path.dirname(dockerfile)
        if parent:
            hints.append(directory_name(parent).lower())

    return unique(hint.lower() for hint in hints if hint)


def find_matching_directory_service(item, services):
    """Find an already-detected (directory-scanned) service that this
    compose service definition actually refers to, so we don't add a
    second, duplicate entry for the same physical service."""
    ...
    hints = compose_service_target_hints(item)
    if not hints:
        return None

    for service in services:
        directory = clean_path(service.get("directory", "")).strip("/")
        candidates = {service["name"].lower()}
        if directory:
            candidates.add(directory_name(directory).lower())

        if candidates.intersection(hints):
            return service
...
def detect_services(files, archive, compose_services, k8s_workloads):
    ...
    for item in compose_services:
        matched = find_matching_directory_service(item, services)

        if matched:
            # This compose service is the same physical service as one we
            # already found by scanning directories - merge instead of
            # adding a duplicate (which would double-count it in every
            # downstream risk/score/architecture calculation).
            matched["ports"] = unique(matched.get("ports", []) + item.get("ports", []))
            matched["depends_on"] = unique(matched.get("depends_on", []) + item.get("depends_on", []))
            matched.setdefault("compose_names", [])
            if item["name"] not in matched["compose_names"]:
                matched["compose_names"].append(item["name"])
            if item.get("replicas", 1) > 1:
                matched["declared_replicas"] = item["replicas"]
            continue

        name_lower = item["name"].lower()
        if name_lower in {service["name"].lower() for service in services}:
            continue

        service_type = (
            "frontend" if name_lower in FRONTEND_NAMES else
            "worker" if name_lower in WORKER_NAMES else
            "backend"
        )
        ...
        services.append({
            ...
            "technology": technology,
            "type": service_type,
            "directory": item.get("build_context") or item.get("build") or "",
            "frameworks": [],
            "ports": item.get("ports", []),
            "depends_on": item.get("depends_on", []),
            **({"declared_replicas": item["replicas"]} if item.get("replicas", 1) > 1 else {})
        })
    ...
    for item in k8s_workloads:
        ...
        name_lower = item["name"].lower()
        service_type = (
            "worker" if any(name in name_lower for name in WORKER_NAMES) else
            "frontend" if any(name in name_lower for name in FRONTEND_NAMES) else
            "backend"
        )
        services.append({
            "name": item["name"],
    ...
    seen = set()
    for service in services:
        key = (service["name"].lower(), service["type"], service.get("directory", ""))
        if key not in seen:
            seen.add(key)
...
def collect_project_text(files, archive, max_total=12_000_000):
    chunks = []
    total = 0
    for file in files:
        if is_ignored(file):
...
def detect_dependencies(files, archive):
    ...
    evidence = {"databases": {}, "queues": {}, "caches": {}, "object_storage": {}}
    ...
    for match in re.findall(r"https?://[A-Za-z0-9._:-]+", text):
        try:
            ...
            pass
    dependencies["external_services"] = sorted(urls)[:50]
            ...
            for raw in content.splitlines():
                ...
                if SECRET_NAME_PATTERNS.search(key):
                    potential_secrets.append({
                        "file": file, "variable": key, "value_exposed": bool(value.strip())
                    })

    return {"files": unique(env_files), "variables": unique(variables), "potential_secrets": potential_secrets}
...
def detect_service_health(services, files, archive):
    ...
    for service in services:
        directory = clean_path(service.get("directory", "")).strip("/")
        if directory in ("", "."):
            service_files = [file for file in files if "/" not in file]
        else:
            service_files = [file for file in files if file.startswith(directory + "/")]
...
def detect_docker(files):
    return [file for file in files if is_dockerfile_name(basename(file))]
...
def detect_ci_cd(files):
    ...
    for file in files:
        ...
        if (
            ...
            or basename(path) in {"jenkinsfile", "azure-pipelines.yml", "circleci"}
        ):
            results.append(file)
    return results
...
def detect_service_urls(services, files, archive):
    ...
    for service in services:
        directory = clean_path(service.get("directory", "")).strip("/")
        if directory in ("", "."):
            relevant = [file for file in files if "/" not in file]
        else:
            relevant = [file for file in files if file.startswith(directory + "/")]
        ...
        for variable in unique(URL_ENV_PATTERN.findall(text)):
            if any(token in variable for token in (
                "DATABASE", "DB_", "REDIS", "S3", "AWS_", "KAFKA", "RABBIT", "QUEUE", "MONGO", "POSTGRES", "MYSQL"
            )):
                continue
...
def build_architecture(services, dependencies, compose_services, k8s_workloads):
    ...
    for database in dependencies["databases"]:
        nodes.append({"name": database.lower(), "type": "database", "technology": database})
    for queue in dependencies["queues"]:
        nodes.append({"name": queue.lower().replace(" ", "-"), "type": "queue", "technology": queue})
    for cache in dependencies["caches"]:
        nodes.append({"name": cache.lower(), "type": "cache", "technology": cache})
    for storage in dependencies["object_storage"]:
        nodes.append({"name": storage.lower().replace(" ", "-"), "type": "object_storage", "technology": storage})

    frontends = [service for service in services if service["type"] == "frontend"]
    backends = [service for service in services if service["type"] == "backend"]
    workers = [service for service in services if service["type"] == "worker"]

    for frontend in frontends:
        for backend in backends:
            connections.append({
                "from": frontend["name"], "to": backend["name"],
                "relationship": "request", "inference": "static architecture inference"
            })

    for backend in backends:
        for worker in workers:
            connections.append({
                "from": backend["name"], "to": worker["name"],
                "relationship": "processing", "inference": "static architecture inference"
            })
        for database in dependencies["databases"]:
            connections.append({
                "from": backend["name"], "to": database.lower(),
                "relationship": "database", "inference": "dependency detection"
            })
        for queue in dependencies["queues"]:
            connections.append({
                "from": backend["name"], "to": queue.lower().replace(" ", "-"),
                "relationship": "queue", "inference": "dependency detection"
            })
        for cache in dependencies["caches"]:
            connections.append({
                "from": backend["name"], "to": cache.lower(),
                "relationship": "cache", "inference": "dependency detection"
            })

    for compose in compose_services:
        for dependency in compose.get("depends_on", []):
            connections.append({
                "from": compose["name"], "to": dependency,
                "relationship": "depends_on", "inference": "Docker Compose"
            })
...
def build_failure_scenarios(services, dependencies):
    ...
    for service in services:
        ...
        scenarios.append({
            "id": service["name"], "type": "service_failure", "target": service["name"],
            "label": f"{service['name']} Failure",
            "description": f"Simulate failure of the {service_type} service."
        })

    for database in dependencies["databases"]:
        scenarios.append({
            "id": f"database:{database}", "type": "database_failure", "target": database,
            "label": f"{database} Failure",
            "description": f"Simulate loss of the {database} dependency."
        })

    for queue in dependencies["queues"]:
        scenarios.append({
            "id": f"queue:{queue}", "type": "queue_failure", "target": queue,
            "label": f"{queue} Failure",
            "description": f"Simulate loss of the {queue} messaging layer."
        })

    for cache in dependencies["caches"]:
        scenarios.append({
            "id": f"cache:{cache}", "type": "cache_failure", "target": cache,
            "label": f"{cache} Failure",
            "description": f"Simulate loss of the {cache} cache layer."
        })

    scenarios.append({
        "id": "traffic", "type": "traffic_spike", "target": "all",
        "label": "Traffic Spike \u00d710",
        "description": "Simulate a tenfold increase in incoming traffic."
    })

    scenarios.append({
        "id": "cascading", "type": "cascading_failure", "target": "critical-path",
...
def make_finding(severity, category, title, description, recommendation, score_impact):
    return {
        ...
        "description": description, "recommendation": recommendation, "score_impact": score_impact
    }
    ...
    for service in services:
        ...
        if service["type"] == "backend" and replicas < 2:
            findings.append(make_finding(
                "critical", "availability", f"{service['name']} Single Point of Failure",
                f"Only one instance is statically represented for {service['name']}. A single instance can become a complete request-path failure.",
                f"Run at least 2 {service['name']} instances behind a health-aware load balancer.", 25
            ))
        elif service["type"] == "worker" and replicas < 2:
            findings.append(make_finding(
                "warning", "availability", f"{service['name']} Worker Redundancy Not Detected",
                f"No redundant {service['name']} worker instances were detected.",
                f"Run multiple {service['name']} workers and use durable queue semantics where applicable.", 10
            ))
        elif service["type"] == "frontend" and replicas < 2 and len(frontends) == 1:
            findings.append(make_finding(
                "warning", "availability", f"{service['name']} Frontend Redundancy Not Detected",
                "Only one frontend service/instance is represented.",
                "Use redundant frontend instances or a highly available static/CDN deployment.", 8
            ))

    if not health_endpoints:
        findings.append(make_finding(
            "warning", "observability", "Health Check Not Detected",
            "No common application health endpoint was detected.",
            "Expose /health or /healthz and configure deployment health checks.", 10
        ))
    else:
        missing = [
            service["name"] for service in services
            if service["type"] in {"backend", "worker"} and service["name"] not in service_health
        ]
        if missing:
            findings.append(make_finding(
                "warning", "observability", "Some Services Lack Health Checks",
                "Health endpoints exist somewhere in the project, but not every backend/worker service has a service-specific endpoint detected.",
                "Add service-specific health/readiness checks where appropriate.", 5
            ))

    for database in dependencies["databases"]:
        findings.append(make_finding(
            "warning", "data", f"{database} Availability Must Be Reviewed",
            f"{database} is a detected application dependency. Static analysis cannot confirm whether the database is replicated or highly available.",
            f"Review {database} backups, replication, failover, connection pooling and recovery requirements.", 8
        ))

    if workers and not dependencies["queues"]:
        findings.append(make_finding(
            "warning", "async", "Worker Queue Not Detected",
            "Background worker-like services were found but no obvious durable messaging infrastructure was detected.",
            "Consider a durable queue to absorb bursts and isolate producer/consumer failures.", 8
        ))

    if not dockerfiles and not compose_services and not k8s_workloads:
        findings.append(make_finding(
            "info", "deployment", "Container / Orchestration Configuration Not Detected",
            "No Dockerfile, Compose service definition or Kubernetes workload was detected.",
            "Add explicit deployment configuration for reproducible production deployment.", 3
        ))

    if zerops_configs:
        findings.append(make_finding(
            "info", "deployment", "Existing Zerops Configuration Detected",
            "A Zerops configuration file already exists in the project.",
            "Compare the existing configuration with the generated optimization plan.", 0
        ))

    exposed_secrets = [item for item in env_info["potential_secrets"] if item["value_exposed"]]
    if exposed_secrets:
        findings.append(make_finding(
            "critical", "security", "Potential Secrets Found in Environment Files",
            f"{len(exposed_secrets)} secret-like environment variables appear to have values in uploaded files.",
            "Do not commit real credentials. Rotate exposed secrets and use deployment secret management.", 20
        ))

    if not ci_cd:
        findings.append(make_finding(
            "info", "delivery", "CI/CD Configuration Not Detected",
            "No common CI/CD workflow was detected.",
            "Add automated build, test and deployment checks before production rollout.", 3
        ))

    if not iac:
        findings.append(make_finding(
            "info", "infrastructure", "Infrastructure-as-Code Not Detected",
            "No Terraform/Pulumi-style infrastructure configuration was detected.",
            "Consider versioning infrastructure configuration when infrastructure complexity grows.", 2
        ))

    if not services:
        findings.append(make_finding(
            "critical", "analysis", "Application Service Could Not Be Identified",
            "The analyzer could not confidently identify an application service.",
            "Use conventional service directories, a supported manifest, Compose, or Kubernetes configuration.", 30
        ))
...
def detect_bottlenecks(services, dependencies, findings, architecture):
    ...
    for service in services:
        if service["type"] == "backend":
            candidates.append({
                "component": service["name"], "type": "service", "risk": "high",
                "reason": "Backend services sit on the primary request path and are treated as single-instance unless deployment metadata says otherwise.",
                "priority": 90
            })

    for database in dependencies["databases"]:
        candidates.append({
            "component": database, "type": "database", "risk": "high",
            "reason": "Detected database dependency can constrain availability and throughput.", "priority": 85
        })

    for queue in dependencies["queues"]:
        candidates.append({
            "component": queue, "type": "queue", "risk": "medium",
            "reason": "Messaging infrastructure can become a throughput or backlog bottleneck.", "priority": 70
        })

    for cache in dependencies["caches"]:
        candidates.append({
            "component": cache, "type": "cache", "risk": "medium",
            "reason": "Cache availability or saturation can affect latency and dependency load.", "priority": 60
        })

    for item in findings:
        if item["severity"] == "critical":
            candidates.append({
                "component": item["title"], "type": "finding", "risk": "critical",
                "reason": item["description"], "priority": 95
            })
...
def build_optimization_plan(services, dependencies, findings, deployment):
    ...
    for service in services:
        ...
        if service["type"] in {"backend", "worker", "frontend"} and replicas < 2:
            actions.append({
                ...
                "service": service["name"], "action": "Increase redundancy",
                "current": f"{replicas} detected instance(s)", "recommended": "2+ instances",
                "reason": "Reduces the chance that one instance failure takes the service offline."
            })

        if service["name"] not in deployment.get("service_health", {}) and service["type"] in {"backend", "worker"}:
            actions.append({
                "priority": "medium", "service": service["name"], "action": "Add health/readiness checks",
                "current": "No service-specific health endpoint detected", "recommended": "/health or /healthz",
                "reason": "Unhealthy instances can be removed from traffic."
            })

    if dependencies["databases"]:
        actions.append({
            "priority": "high", "service": "database layer", "action": "Review database high availability",
            "current": ", ".join(dependencies["databases"]),
            "recommended": "Backups + replication/failover appropriate to workload",
            "reason": "Application redundancy does not remove a database dependency bottleneck."
        })

    if any(service["type"] == "worker" for service in services) and not dependencies["queues"]:
        actions.append({
            "priority": "medium", "service": "worker infrastructure", "action": "Introduce durable queueing",
            "current": "No queue detected", "recommended": "Durable queue + retry/dead-letter strategy",
            "reason": "Separates request producers from background consumers."
        })

    if not deployment.get("dockerfiles") and not deployment.get("compose_files") and not deployment.get("kubernetes_files"):
        actions.append({
            "priority": "medium", "service": "deployment", "action": "Add explicit deployment configuration",
            "current": "No container/orchestration configuration detected",
    ...
    if not deployment.get("ci_cd"):
        actions.append({
            "priority": "low", "service": "delivery", "action": "Add CI/CD validation",
            "current": "No CI/CD configuration detected",
            "recommended": "Automated build + test + deployment checks",
            "reason": "Prevents known reliability problems from reaching production."
        })

    return actions


def zerops_technology(service):
...
def build_summary(services, technologies, dependencies, score, risk_summary, bottlenecks):
    service_counts = {
        "frontend": len([service for service in services if service["type"] == "frontend"]),
        "backend": len([service for service in services if service["type"] == "backend"]),
        "worker": len([service for service in services if service["type"] == "worker"])
    }
...
def home():
    return jsonify({
        "service": "Zerops Autopilot Analyzer", "status": "running", "version": VERSION,
        "mode": "static-analysis", "ai_required": False, "database_required": False
    })
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as archive:
                files = [
                    clean_path(name) for name in archive.namelist()
                    if not name.endswith("/") and clean_path(name) and not is_ignored(name)
                ]
                ...
                dependencies, dependency_evidence = detect_dependencies(files, archive)
                ...
                architecture = build_architecture(services, dependencies, compose_services, k8s_workloads)
                findings = analyze_reliability(
                    services, dependencies, health_endpoints, service_health, dockerfiles,
                    zerops_configs, compose_services, k8s_workloads, env_info, ci_cd, iac
                )
                reliability_score = calculate_score(findings)
                risk_summary = calculate_risk_summary(findings)
                bottlenecks = detect_bottlenecks(services, dependencies, findings, architecture)

                deployment = {
                    "ports": ports, "health_endpoints": health_endpoints,
                    "health_endpoints_by_service": service_health, "service_health": service_health,
                    "dockerfiles": dockerfiles, "zerops_configs": zerops_configs,
                    "compose_files": compose_files, "kubernetes_files": kubernetes_files,
                    "ci_cd": ci_cd, "iac": iac, "environment_files": env_info["files"]
                }

                optimization_plan = build_optimization_plan(services, dependencies, findings, deployment)
                ...
                response = {
                    "status": "success",
                    "analyzer": {
                        "name": "Zerops Autopilot Analyzer", "version": VERSION, "mode": "static-analysis",
                        "ai_required": False, "database_required": False
                    },
                    ...
                    "environment": {
                        "files": env_info["files"], "variable_count": len(env_info["variables"]),
                        "variables": env_info["variables"][:200], "potential_secrets": env_info["potential_secrets"]
                    },
            ...
            app.logger.info(
                ...
                project.filename, len(files), len(services),
                reliability_score, risk_summary["level"]
            )
